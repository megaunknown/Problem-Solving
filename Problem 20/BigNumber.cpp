...
char* BigNumber::shiftLeftColumn(char* szBuffer,int iCol)
{	
	char* szTmpBuffer = new char[MAX_ARRAY_SIZE];
	memset(szTmpBuffer,'\0',MAX_ARRAY_SIZE);
	memcpy(szTmpBuffer,szBuffer,stringLength(szBuffer));

	while(iCol != 0)
	{
		strcat(szTmpBuffer,"0");
		iCol-- ;
	}
...
char* BigNumber::shiftRightColumn(char* szBuffer,int iCol)
{	
	char* szRetBuffer = new char[MAX_ARRAY_SIZE];
	char* szZeros = new char[MAX_ARRAY_SIZE];

	memset(szRetBuffer,'\0',MAX_ARRAY_SIZE);
	memcpy(szRetBuffer,ReverseStr(szBuffer),stringLength(szBuffer)+1);
	memset(szZeros,'\0',MAX_ARRAY_SIZE);
	...
	strcat(szRetBuffer,szZeros);
	szRetBuffer = ReverseStr(szRetBuffer);
...
//	cout << strValue << " "<< "Multiply by :" << num << endl;
	...
	else
	{
		...
		//if Given and Stored lengths are equal set j,j equal to anyone of them -1 to start from the right side
		if(iLenGiven == iLenStored)
		{
			i = j = iLenGiven - 1 ;
		}
