...
ArrayList<int>* genALPrimeBelowN(int N)
{
	ArrayList<int>* myAL = new ArrayList<int>(N);
	for(int i = 2 ; i < N; i++)
	{
		if(isPrime(i))
		{
	...
	i = myAL->getNumofElements();
	int counter = 0 ;
	ArrayList<int>* myALEx = new ArrayList<int>(i);
	
	myAL->moveFirst();
	while (counter < i )
	{
		myALEx->insert(myAL->getElement());
		myAL->moveNext();
		counter++;
	}
...
*/
	/*
	ArrayList<int> nx = genALPrimeBelowN(1000000);
	nx.moveFirst();
//	nx.print();

	cout << retSquence(nx,958577) << endl;
	*/
	/*
	ArrayList<int> nx = genALPrimeBelowN(100);
	nx.moveFirst();
//	nx.print();

	cout << retSquence(nx,17) << endl;
*/
